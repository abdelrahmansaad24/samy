  ...
  courses: [
    {
      ...
      platform: "Udemy",
      date: "2023",
      desc: "Mastering Python libraries like Pandas, NumPy, and Matplotlib for data manipulation.",
      link: "#"
    }
  ],
  experience: {
  ...
  contact: [
    {
      id: 1,
      type: "Phone & WhatsApp",
      value: "+20 111 871 6536",
      link: "https://wa.me/201118716536",
      icon: "Phone"
    },
    {
      id: 2,
      type: "LinkedIn",
      value: "Mohamed Samy",
      link: "https://www.linkedin.com/in/mohamed-samy-637b14274/",
      icon: "Linkedin"
    },
    {
      id: 3,
      type: "Email",
      value: "admin@MS2004.com",
      link: "mailto:admin@MS2004.com",
      icon: "Mail"
    }
  ]
};
...
const EditableText = ({ text, isEditing, onChange, className, type = "input", placeholder = "", isDarkMode = true, label = "" }) => {
  // Safe rendering
  ...
  return (
    <div className="w-full">
      {label && <span className="text-[10px] uppercase tracking-wider opacity-50 mb-1 block">{label}</span>}
      {type === "textarea" ? (
        <textarea
          value={safeText}
          onChange={(e) => onChange(e.target.value)}
          className={`${inputBgClass} border rounded p-2 w-full focus:outline-none focus:ring-1 focus:ring-emerald-500 text-sm ${className}`}
          rows={4}
        ...
        <input
          value={safeText}
          onChange={(e) => onChange(e.target.value)}
          className={`${inputBgClass} border rounded px-2 py-1 w-full focus:outline-none focus:ring-1 focus:ring-emerald-500 text-sm ${className}`}
          placeholder={placeholder}
        />
      )}
    </div>
...
const IconMap = ({ name, className }) => {
  const icons = { 
    BarChart, Database, Terminal, Code, Brain, 
    Phone, Linkedin, Mail, MessageCircle, Github, 
    Globe, Twitter, Instagram, Facebook, MapPin, 
    BookOpen, Briefcase, GraduationCap 
  };
  // Fallback to BarChart if name doesn't match
...
export default function App() {
    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          // Deep merge or overwrite? Let's spread. 
          // Note: If remote is missing fields (like courses), they might be undefined if we just overwrite.
          // Better to spread defaultData then remoteData
          setData(prev => ({ ...defaultData, ...remoteData }));
        }
      },
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-emerald-500 selection:text-white transition-colors duration-300 ${theme.bg} ${theme.text}`}>
      ...
      <section id="courses" className={`py-20 ${theme.bgAlt}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-12">
            ...
            {isEditing && (
              <button 
                onClick={() => addArrayItem('courses', { id: Date.now(), title: "New Course", platform: "Platform", date: "Year", desc: "Description", link: "#" })}
                className="flex items-center text-emerald-500 text-sm hover:text-emerald-400"
              >
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {(data.courses || []).map((course, index) => (
               <div key={course.id} className={`relative group p-6 rounded-xl border transition-all duration-300 ${theme.bg} ${theme.border} ${theme.cardHover}`}>
                 {isEditing && (
                  <button 
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                  <div className="flex items-center w-full md:w-auto">
                    <BookOpen className="text-emerald-500 mr-3 flex-shrink-0" size={24} />
                    <h3 className={`text-xl font-bold w-full ${theme.textHeading}`}>
                      <EditableText isEditing={isEditing} isDarkMode={isDarkMode} text={course.title} onChange={(v) => updateArrayItem('courses', index, 'title', v)} />
                    </h3>
                  </div>
                  <span className={`${theme.textMuted} text-sm mt-2 md:mt-0 whitespace-nowrap md:ml-4`}>
                    <EditableText isEditing={isEditing} isDarkMode={isDarkMode} text={course.date} onChange={(v) => updateArrayItem('courses', index, 'date', v)} />
                  </span>
                </div>
                 <p className="text-emerald-500 mb-2 font-medium">
                  <EditableText isEditing={isEditing} isDarkMode={isDarkMode} text={course.platform} onChange={(v) => updateArrayItem('courses', index, 'platform', v)} />
                </p>
                <p className={`${theme.textMuted} mb-4`}>
                  <EditableText type="textarea" isEditing={isEditing} isDarkMode={isDarkMode} text={course.desc} onChange={(v) => updateArrayItem('courses', index, 'desc', v)} />
                </p>
                 <a 
                  href={course.link} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className={`inline-flex items-center text-sm font-medium hover:text-emerald-500 ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}
                >
                  <ExternalLink size={14} className="mr-1" /> View Certificate
                  {isEditing && <span className="ml-2 text-xs opacity-50">(Edit link in edit mode)</span>}
                  <EditableText isEditing={isEditing} isDarkMode={isDarkMode} text={course.link} onChange={(v) => updateArrayItem('courses', index, 'link', v)} className="ml-2" />
                </a>
               </div>
            ))}
          </div>
      ...
      {/* Experience Section */}
      <section id="experience" className={`py-20 ${theme.bg}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* Projects Section */}
      <section id="projects" className={`py-20 ${theme.bgAlt}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-12">
            <h2 className={`text-3xl font-bold flex items-center ${theme.textHeading}`}>
              <span className="w-12 h-1 bg-emerald-500 mr-4"></span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.projects.map((project, index) => (
              <div key={project.id} className={`rounded-xl border p-6 transition-all group relative flex flex-col ${theme.bg} ${theme.border} ${theme.cardHover}`}>
                {isEditing && (
                  <button 
                ...
                >
                  View Project <ExternalLink size={16} className="ml-2" />
                   {isEditing && <span className="ml-2 text-xs opacity-50 w-full"><EditableText isEditing={true} isDarkMode={isDarkMode} text={project.link} onChange={(v) => updateArrayItem('projects', index, 'link', v)} placeholder="URL" /></span>}
                </a>
              </div>
            ))}
          </div>
      ...
      {/* Skills Section */}
      <section id="skills" className={`py-20 ${theme.bg}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className={`text-3xl font-bold mb-12 flex items-center ${theme.textHeading}`}>
            <span className="w-12 h-1 bg-emerald-500 mr-4"></span>
            SKILLS
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {/* Technical Skills */}
            <div className={`p-8 rounded-xl border ${theme.bgAlt} ${theme.border}`}>
            ...
            <div className={`p-8 rounded-xl border ${theme.bgAlt} ${theme.border}`}>
              ...
              <ul className="space-y-4">
                {data.skills.soft.map((skill, index) => (
                  <li key={index} className={`p-4 rounded border group relative ${theme.bg} ${theme.border}`}>
                     {isEditing && (
                      <button 
                        onClick={() => {
                           const newData = JSON.parse(JSON.stringify(data));
      ...
      {/* Services Section */}
      <section id="services" className={`py-20 ${theme.bgAlt}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className={`text-3xl font-bold mb-12 flex items-center ${theme.textHeading}`}>
            <span className="w-12 h-1 bg-emerald-500 mr-4"></span>
            SERVICES
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {data.services.map((service, index) => (
              <div key={index} className={`p-6 rounded-lg border transition-colors group ${theme.bg} ${theme.border} ${theme.cardHover}`}>
      ...
      {/* Contact Section (Customizable) */}
      <section id="contact" className={`py-24 relative overflow-hidden ${theme.bg}`}>
        {/* Background glow */}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-4xl mx-auto">
             {/* Map through dynamic contact methods */}
             {Array.isArray(data.contact) && data.contact.map((item, index) => (
                <div key={item.id || index} className={`relative flex flex-col items-center justify-center p-6 border rounded-xl transition-all group ${theme.bgAlt} ${theme.border} ${theme.cardHover}`}>
                   {isEditing && (
                      <button 
                        onClick={() => removeArrayItem('contact', index)}
                        className="absolute top-2 right-2 text-neutral-600 hover:text-red-500 opacity-50 hover:opacity-100 z-10"
                      >
                        <Trash2 size={16} />
                      </button>
                   )}
                   
                   <div className="mb-3 text-emerald-500 group-hover:scale-110 transition-transform">
                      <IconMap name={item.icon} className="h-8 w-8" />
                   </div>
                   
                   <div className="text-center w-full">
                      <span className={`block text-xs uppercase tracking-wider mb-1 ${theme.textMuted}`}>
                         <EditableText isEditing={isEditing} isDarkMode={isDarkMode} text={item.type} onChange={(v) => updateArrayItem('contact', index, 'type', v)} placeholder="Type (e.g., Phone)" />
                      </span>
                      <span className={`font-medium block mb-2 ${theme.textHeading}`}>
                         <EditableText isEditing={isEditing} isDarkMode={isDarkMode} text={item.value} onChange={(v) => updateArrayItem('contact', index, 'value', v)} placeholder="Display Value" />
                      </span>
                      {isEditing ? (
                        <div className="space-y-2 mt-2 border-t border-dashed pt-2 border-neutral-700">
                          <EditableText isEditing={true} isDarkMode={isDarkMode} text={item.link} onChange={(v) => updateArrayItem('contact', index, 'link', v)} placeholder="Link URL" label="Link" />
                          <EditableText isEditing={true} isDarkMode={isDarkMode} text={item.icon} onChange={(v) => updateArrayItem('contact', index, 'icon', v)} placeholder="Icon Name" label="Icon Name" />
                        </div>
                      ) : (
                         <a 
                           href={item.link} 
                           target="_blank" 
                           rel="noopener noreferrer"
                           className="inline-block px-4 py-2 text-xs font-bold rounded-full bg-emerald-500 text-white hover:bg-emerald-600 transition-colors"
                         >
                           Connect
                         </a>
                      )}
             ...
             {isEditing && (
                <button 
                  onClick={() => addArrayItem('contact', { id: Date.now(), type: "New Method", value: "Display Value", link: "#", icon: "Globe" })}
                  className={`flex flex-col items-center justify-center p-6 border border-dashed rounded-xl transition-all ${theme.border} hover:border-emerald-500 text-emerald-500`}
                >
      ...
      <footer className={`py-8 border-t text-center relative ${theme.bgAlt} ${theme.border}`}>
